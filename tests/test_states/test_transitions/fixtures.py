...
class MMachineStat(IMachineStatistics):
...
def fsm_and_ddosing_boys() -> FSMAndDdosingBoys:
    ...
    fsm = StateMachine(factory.make_initial(ddosing), asyncio.get_event_loop(), MMachineStat())
...
def fsm_and_failing_boys() -> FSMAndBoys:
    ...
    fsm = StateMachine(factory.make_initial(), asyncio.get_event_loop(), MMachineStat())
...
def fsm_and_penetrating_this_month_boys() -> FSMAndPenetratingBoys:
    ...
    fsm = StateMachine(factory.make_ddosing(penetration), asyncio.get_event_loop(), MMachineStat())
...
def fsm_and_failed_penetration_boys() -> FSMAndDdosingBoys:
    ...
    fsm = StateMachine(factory.make_penetration(ddosing=ddosing), asyncio.get_event_loop(), MMachineStat())
    return FSMAndDdosingBoys(fsm, actions, factory, ddosing)
...
def fsm_and_penetrated_boys() -> FSMAndBoys:
    ...
    fsm = StateMachine(factory.make_penetration(), asyncio.get_event_loop(), MMachineStat())
    return FSMAndBoys(fsm, actions, factory)
