...
def test_shutdown_and_next_start_times():
    now = datetime.now().replace(hour=10, microsecond=0)
    clocks = OperatorClocks(now=now)
    shutdown, start = clocks.shutdown_and_next_start_times(now.time())
    assert shutdown == datetime.now().replace(hour=20, microsecond=0)
    assert start == datetime.now().replace(day=datetime.now().day + 1, hour=8, microsecond=0)
